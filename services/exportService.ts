...
import { Pattern, DrumMachineSound } from '../types';
import { INSTRUMENTS_ORDER, GENERAL_MIDI_DRUM_MAP, MIDI_TICKS_PER_QUARTER_NOTE } from '../constants';
...
export class ExportService {
  ...
  public exportPatternToMidi(pattern: Pattern): void {
    ...
    if (!window.MIDIWriter) {
      console.error('MIDIWriter library (window.MIDIWriter) is not loaded.');
    ...
    INSTRUMENTS_ORDER.forEach(instrument => {
      ...
      pattern.steps[instrument].forEach((isActive, stepIndex) => {
        if (isActive) {
